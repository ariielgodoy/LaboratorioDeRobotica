###########################################################################
## Makefile generated for component 'evitar_choque2023v'. 
## 
## Makefile     : evitar_choque2023v.mk
## Generated on : Sun Oct 20 13:08:43 2024
## Final product: $(RELATIVE_PATH_TO_ANCHOR)/evitar_choque2023v.elf
...
PRODUCT_NAME              = evitar_choque2023v
MAKEFILE                  = evitar_choque2023v.mk
...
PRODUCT = $(RELATIVE_PATH_TO_ANCHOR)/evitar_choque2023v.elf
...
INCLUDES_BUILDINFO = -I$(START_DIR) -IC:/PROGRA~3/MATLAB/SUPPOR~1/R2023b/toolbox/target/SUPPOR~1/ARDUIN~2/include -I$(MATLAB_ROOT)/toolbox/target/shared/svd/common/include -I$(START_DIR)/evitar_choque2023v_ert_rtw -I$(MATLAB_ROOT)/extern/include -I$(MATLAB_ROOT)/simulink/include -I$(MATLAB_ROOT)/rtw/c/src -I$(MATLAB_ROOT)/rtw/c/src/ext_mode/common -I$(MATLAB_ROOT)/rtw/c/ert -I$(ARDUINO_ROOT)/hardware/arduino/avr/cores/arduino -I$(ARDUINO_ROOT)/hardware/arduino/avr/variants/mega -IC:/PROGRA~3/MATLAB/SUPPOR~1/R2023b/toolbox/target/SUPPOR~1/ARDUIN~1/include -IC:/PROGRA~3/MATLAB/SUPPOR~1/R2023b/toolbox/target/SUPPOR~1/ARDUIN~1/SCHEDU~1/include -I$(ARDUINO_ROOT)/hardware/tools/avr/avr/include/avr
...
DEFINES_STANDARD = -DMODEL=evitar_choque2023v -DNUMST=1 -DNCSTATES=0 -DHAVESTDIO -DMODEL_HAS_DYNAMICALLY_LOADED_SFCNS=0
...
OBJS = MW_arduino_digitalio.o MW_PWM.o MW_PWMDriver.o ArduinoPinHandleMap.o MW_AnalogInput.o evitar_choque2023v.o evitar_choque2023v_data.o rtGetInf.o rtGetNaN.o rt_nonfinite.o MW_ArduinoHWInit.o io_wrappers.o arduinoAVRScheduler.o
...
%.o : $(START_DIR)/evitar_choque2023v_ert_rtw/%.c
	$(CC) $(CFLAGS) -o "$@" $(subst /,\,"$<")


%.S.o : $(START_DIR)/evitar_choque2023v_ert_rtw/%.s
	$(AS) $(ASFLAGS) -o "$@" $(subst /,\,"$<")
...
%.o : $(START_DIR)/evitar_choque2023v_ert_rtw/%.cpp
	$(CPP) $(CPPFLAGS) -o "$@" $(subst /,\,"$<")
...
ert_main.o : $(START_DIR)/evitar_choque2023v_ert_rtw/ert_main.c
	$(CC) $(CFLAGS) -o "$@" $(subst /,\,"$<")


evitar_choque2023v.o : $(START_DIR)/evitar_choque2023v_ert_rtw/evitar_choque2023v.c
	$(CC) $(CFLAGS) -o "$@" $(subst /,\,"$<")


evitar_choque2023v_data.o : $(START_DIR)/evitar_choque2023v_ert_rtw/evitar_choque2023v_data.c
	$(CC) $(CFLAGS) -o "$@" $(subst /,\,"$<")


rtGetInf.o : $(START_DIR)/evitar_choque2023v_ert_rtw/rtGetInf.c
	$(CC) $(CFLAGS) -o "$@" $(subst /,\,"$<")


rtGetNaN.o : $(START_DIR)/evitar_choque2023v_ert_rtw/rtGetNaN.c
	$(CC) $(CFLAGS) -o "$@" $(subst /,\,"$<")


rt_nonfinite.o : $(START_DIR)/evitar_choque2023v_ert_rtw/rt_nonfinite.c
	$(CC) $(CFLAGS) -o "$@" $(subst /,\,"$<")
